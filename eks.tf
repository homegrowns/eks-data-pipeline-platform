module "eks" {
  ...
  cluster_endpoint_public_access_cidrs = [
    ...
    "222.106.16.201/32",#역삼 스타벅스
    "112.172.181.110/32" # main h
  ]
...
eks_managed_node_groups = {
    # airflow 그룹: 스케줄러와 웹서버가 살 곳 (안정성 중요!)
    airflow = {
      name = "airflow"
      ...
      # 인스턴스 타입 믹싱 (Instance Diversity) 기법
      instance_types = ["t3.medium"]
    ...
    kafka = {
      ...
      # 헬름 차트에서 nodeSelector로 이 라벨을 찾게 함
      labels = {
        role       = "data-worker" # 카프카, 스파크 공통 이름
        node-class = "ec2"
      }
...
locals {
  ...
  cluster_name = "eks-malware-${random_string.suffix.result}"
}